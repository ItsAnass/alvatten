...
@register.simple_tag
def get_random_fountain_images(count=15):
    # Path to the fountain folder in static files
    fountain_dir = os.path.join(settings.BASE_DIR, 'alvattenapp', 'static', 'img', 'fountain')
    image_extensions = ('.jpg', '.jpeg', '.png', '.gif', '.JPG', '.JPEG', '.PNG', '.GIF')
    image_paths = []
    for root, dirs, files in os.walk(fountain_dir):
        for file in files:
