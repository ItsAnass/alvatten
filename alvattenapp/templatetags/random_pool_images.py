...
@register.simple_tag
def get_random_pool_images(count=15):
    # Path to the pooler folder in static files
    pooler_dir = os.path.join(settings.BASE_DIR, 'alvattenapp', 'static', 'img', 'pooler')
    image_extensions = ('.jpg', '.jpeg', '.png', '.gif', '.JPG', '.JPEG', '.PNG', '.GIF')
    image_paths = []
    for root, dirs, files in os.walk(pooler_dir):
        for file in files:
